...
$html = new html("TORNEO DI RITORNO");
$html->title("Torneo di Ritorno",_TRLOGO);

$Gironi = array('A','B','C','D');
if($sorteggiato)
{
...
if ($gir<$num_gironi)
{
$html->tabletitle('Classifica girone <a href="torneoritorno.php?g='.($gir-1).'"><</a> '.$Gironi[$gir].' <a href="torneoritorno.php?g='.($gir+1).'">></a> ');
echo <<< main
	<table class="table">
		<tr>
			<th>pos</th>
...
$html->tabletitle('Fase finale <a href="torneoritorno.php?g=2"><</a> <a href="torneoritorno.php?g=4">></a> ');
//$html->tabletitle('Fase finale <a href="torneoritorno.php?g=3"><</a> <a href="torneoritorno.php?g=5">></a> ');
//$html->tabletitle('Fase finale <a href="torneoritorno.php?g=4"><</a> <a href="torneoritorno.php?g=6">></a> ');
...
echo <<< main
	<table class="table">
		...
		<tr>
main;
$result = $db->query('SELECT utenti.id AS id,utenti.squadra AS squadra,utenti.ordine AS ordine,SUM(punti) AS punti FROM formazioni,utenti WHERE formazioni.squadra=utenti.id AND giornata IN (SELECT id FROM giornate WHERE torneo=\'Torneo di Ritorno\' AND id<='.$giornata.') GROUP BY utenti.squadra ORDER BY punti DESC, ordine');
$i = 0;
$tds = count($TDS);
while ($row = $result->fetch_assoc())
{
...
if($gir<$num_gironi)
$html->tabletitle('Calendario girone <a href="torneoritorno.php?g='.($gir-1).'"><</a> '.$Gironi[$gir].' <a href="torneoritorno.php?g='.($gir+1).'">></a> ');
else
$html->tabletitle('Fase finale <a href="torneoritorno.php?g=2"><</a> <a href="torneoritorno.php?g=4">></a> ');
//$html->tabletitle('Fase finale <a href="torneoritorno.php?g=3"><</a> <a href="torneoritorno.php?g=5">></a> ');
//$html->tabletitle('Fase finale <a href="torneoritorno.php?g=4"><</a> <a href="torneoritorno.php?g=6">></a> ');
...
$result = $db->query('SELECT DISTINCT partite.id,s1,s2,u1.squadra AS sq1,u2.squadra AS sq2,gol1,gol2,punti1,punti2,giornata,turno FROM formazioni,partite,utenti AS u1,utenti AS u2 WHERE s1=u1.id AND s2=u2.id AND partita=partite.id AND formazioni.giornata=23 ORDER BY giornata, partite.id');
$i = 0;
$g = 0;
while ($row = $result->fetch_assoc())
{
...
if($gir>=$num_gironi)
{
	$result = $db->query('SELECT DISTINCT partite.id,s1,s2,u1.squadra AS sq1,u2.squadra AS sq2,gol1,gol2,punti1,punti2,giornata,turno FROM formazioni,partite,utenti AS u1,utenti AS u2 WHERE s1=u1.id AND s2=u2.id AND partita=partite.id AND formazioni.giornata=24 ORDER BY giornata, partite.id');
	$i = 0;
	$g = 0;
	while ($row = $result->fetch_assoc())
	{
...
if($gir>=$num_gironi)
{
	$result = $db->query('SELECT DISTINCT partite.id,s1,s2,u1.squadra AS sq1,u2.squadra AS sq2,gol1,gol2,punti1,punti2,giornata,turno FROM formazioni,partite,utenti AS u1,utenti AS u2 WHERE s1=u1.id AND s2=u2.id AND partita=partite.id AND formazioni.giornata=26 ORDER BY giornata, partite.id');
	$i = 0;
	$g = 0;
	while ($row = $result->fetch_assoc())
	{
