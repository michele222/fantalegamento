...
$html = new html("FANTALEGAMENTO CUP");
$html->title("Fantalegamento Cup",_FCLOGO);
...
echo <<< main
	<table class="table">
		...
		<tr>
main;
$result = $db->query('SELECT utenti.id AS id,utenti.squadra AS squadra,utenti.ordine AS ordine,SUM(punti) AS punti FROM formazioni,utenti WHERE formazioni.squadra=utenti.id AND giornata IN (SELECT id FROM giornate WHERE torneo=\'Fantalegamento Cup\' AND id<='.$giornata.') GROUP BY utenti.squadra ORDER BY punti DESC, ordine');
$i = 0;
while ($row = $result->fetch_assoc())
{
	if($i % 2)
